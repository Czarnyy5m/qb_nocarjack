...
function isPlayerJobPolice()
	for k,v in pairs(QBCore.Functions.GetPlayerData()) do
			...
			local lock = GetVehicleDoorLockStatus(veh)
				...
				if ((lock == 7) or (pedd ~= 0 )) then
					if has_value(cfg.job_whitelist, xPlayer.job.name) then
						TriggerServerEvent('qb_nocarjack:setVehicleDoorsForEveryone', {veh, 1, plate})
					else
						if not lucky or blacklisted then
							TriggerServerEvent('qb_nocarjack:setVehicleDoorsForEveryone', {veh, 2, plate})
						else
							TriggerServerEvent('qb_nocarjack:setVehicleDoorsForEveryone', {veh, 1, plate})
						end
					end
...
RegisterNetEvent('qb_nocarjack:setVehicleDoors')
AddEventHandler('qb_nocarjack:setVehicleDoors', function(veh, doors)
